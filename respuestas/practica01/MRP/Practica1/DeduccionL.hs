...
esAxL1 phi = case phi of
  alpha `Oimp`(beta `Oimp` gamma) -> alpha == gamma 
  _ -> False

-- Función que nos dice si una formula de PLI cumple el Axioma 2
esAxL2 :: PLI -> Bool
esAxL2 phi = case phi of
  (alpha `Oimp` (beta `Oimp` gamma)) `Oimp` ((delta `Oimp` epsilon) `Oimp` (zeta `Oimp` eta)) -> (alpha == delta && delta == zeta)
  _ -> False
...
esModusPonens (phi, chi, psi) = case (phi, chi, psi) of
  (alpha, beta `Oimp` gamma, delta) -> (alpha == delta) && (gamma == delta)
  _ -> False
