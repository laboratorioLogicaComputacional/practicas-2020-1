...
esAxL1 phi = case phi of
  Oimp alpha (Oimp beta gamma) -> (alpha == gamma)
  _ -> False

-- Función que nos dice si una formula de PLI cumple el Axioma 2
esAxL2 :: PLI -> Bool
esAxL2 phi = case phi of
  Oimp (Oimp (alpha) (Oimp beta gamma)) (Oimp (Oimp delta eta) (Oimp zeta theta)) -> 
    (alpha == delta) && (alpha == zeta) && (beta == eta) && (gamma == theta)
  _ -> False

-- Función que nos dice si una formula de PLI cumple el Axioma 3
esAxL3 :: PLI -> Bool
esAxL3 phi = case phi of
  Oimp (Oimp (Oimp alpha Bot ) (Oimp beta Bot)) (Oimp gamma delta) -> (alpha == delta) && (beta == gamma)
  _ -> False
...
esModusPonens (phi, chi, psi) = case (phi, chi, psi) of
  (alpha, (Oimp beta gamma) , delta) -> (beta == alpha) && (gamma == delta)
  _ -> False
...
checkPaso lprem lpp p = case p of
  (n, (phi, Prem)) -> phi `elem` lprem -- Revisamos que phi sea parte de lprem
